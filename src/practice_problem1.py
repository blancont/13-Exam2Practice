...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.backup_volume = self.volume
        if len(self.contents) > self.volume:
            self.contents = ''
        self.backup_contents = self.contents
        self.history = []
    ...
    def shrink(self, new_volume):
        ...
        if self.volume < len(self.contents):
            ...
            for k in range(self.volume, len(self.contents)):
                string2 = string2 + self.contents[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        discard1 = len(self.double())
        discard2 = len(self.shrink(new_volume))
    ...
    def reset(self):
        ...
        self.contents = self.backup_contents
        self.volume = self.backup_volume
    ...
    def combined_box(self, other_box):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        box = Box(self.contents, self.volume + other_box.volume)
        box.append_string(other_box.contents)
        return box
